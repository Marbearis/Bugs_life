			...
			switch true do {
				...
				case ((_ins >= 0.2) and
					...
					(alive _en)): {
						...
						_dir = [[0,1,0], -([_mutant, _en] call BIS_fnc_dirTo)] call BIS_fnc_rotateVector2D;
						
						_mutant setVelocityTransformation [ 
						getPosASL _mutant,  
						getPosASL _mutant,  
						[0,0,(velocity _mutant select 2) - 1],  
						[(velocity _mutant select 0),(velocity _mutant select 1),(velocity _mutant select 2) - 1], 
						vectorDir _mutant,  
						_dir,  
						vectorUp _mutant,  
						vectorUp _mutant, 
...
}, selectRandom [7,8,9,10], [_gruntBoy]] call CBA_fnc_addPerFrameHandler;
